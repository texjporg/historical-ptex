...
#define BANNER "This is pTeX, Version p2.1.9, based on TeX, Version 3.14159"
#else /* KANJI */
...
#endif /* TeX */
#ifdef MF
#define BANNER "This is Metafont, Version 2.7182"
#define COPYRIGHT_HOLDER "D.E. Knuth"
#define AUTHOR NULL
...
maininit P2C(int, ac, string *, av)
{
...
#if defined(__STDC__)
  /* Need to delay it because of win32 `-oem' option. Hope it
     is harmless. */
  /* Default value "" means: get value from env. var LC_ALL, LC_CTYPE, or LANG */
  ...
  if (!srcspecialsoption) {
	/* Look in texmf.cnf for a value */
	string src_opt = kpse_var_value("src_specials");
	/* No value means no specials. */
	if (src_opt != NULL) {
	  last_source_name = xstrdup("");
	  parse_src_specials_option(src_opt);
	}
  }
...
parse_options P2C(int, argc,  string *, argv)
{
  ...
  for (;;) {
	...
	else if (ARGUMENT_IS ("stack-size")) {
#endif
#endif /* TeX */
#if defined (TeX) || defined (MF) || defined (MP)
#ifdef WIN32
    } else if (ARGUMENT_IS ("c-style-errors")) {
      extern boolean outputcstyleerrormessagesp;
      outputcstyleerrormessagesp = true;
#endif
#ifndef Omega
...
#if defined(TeX) && !defined(pdfTeX) && !defined(pdfeTeX)
void 
...
#if defined(TeX) && !defined(pdfeTeX) && !defined(pdfTeX)
int
...
#else
  len = strstartar[s + 1 - 65535L] -
	strstartar[s - 65535L];
#endif
  name = xmalloc (len + 1);
#ifndef Omega
  strncpy (name, &strpool[strstart[s]], len);
#else
  strncpy (name, &strpool[strstartar[s - 65535L]], len);
#endif
  name[len] = 0;
  return name;
}
