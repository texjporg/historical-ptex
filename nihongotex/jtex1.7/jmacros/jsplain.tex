% File SPLAIN.TEX - Created 28 October 1986
% 
...
% LaTeX change: this was \centering in PLAIN
...
\def\ialign{\tabskip\z@skip\halign} % initialized \halign
\newcount\mscount
...
\outer\def\proclaim #1. #2\par{\medbreak\vskip-\parskip
...
\def\!{\mskip-\thinmuskip}
\def\*{\discretionary{\thinspace\edef\next{\the\textfont2}\next\char2}{}{}}
...
% LaTeX change: the `\global' in the following command was
% inadvertently omitted by DEK 
{\catcode`\_=\active\global\let_=\_}% _ is like \_ if not used for subscripts
...
\def\angle{{\vbox{\ialign{$\m@th\scriptstyle##$\cr\not\mathrel{\mkern14mu}\cr
      \noalign{\nointerlineskip}
...
\def\ddots{\mathinner{\mskip1mu\raise7\p@\vbox{\kern7\p@\hbox{.}}\mskip2mu
    \raise4\p@\hbox{.}\mskip2mu\raise\p@\hbox{.}\mskip1mu}}
...
\def\bmod{\mskip-\medmuskip\mskip5mu
...
\def\tracingall{\tracingonline\@ne\tracingcommands\tw@\tracingstats\tw@
  ...
  \tracingmacros\@ne\tracingparagraphs\@ne\tracingrestores\@ne
...
% SliTeX: File called JSPLAIN
% identify the current format
\def\fmtname{jsplain}\def\fmtversion{2.09j-8/21/87} 
