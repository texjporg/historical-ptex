...
{integer dotpos, i, a  ; 
  ...
  do 
	...
	else if ( ! foundchange ) 
	{
	  ...
	  {
	    (void) putc('\n',  stdout );
	    (void) Fputs( stdout ,  "! Usage: webfile[.web] [changefile[.ch]]" ) ; 
	  } 
	  error () ; 
	  uexit ( 1 ) ; 
	} 
...
{schar i  ; 
  ...
  {register integer for_end; zi = 0 ; for_end = 2 ; if ( zi <= for_end) do 
    {
      tokstart [ zi ] = 0 ; 
      tokptr [ zi ] = 0 ; 
    } 
...
{/* 31 32 */ register namepointer Result; eightbits c  ; 
  ...
  while ( p != 0 ) {
    ...
    if ( bytestart [ p + 2 ] - bytestart [ p ] == l ) 
    {
      i = idfirst ; 
      k = bytestart [ p ] ; 
      w = p % 2 ; 
  ...
  if ( ( p == nameptr ) || ( t != 0 ) ) 
  {
	...
	while ( q != 0 ) {
	    
	  {
	    ...
	    s = 0 ; 
	    w = q % 2 ; 
	    while ( ( k < bytestart [ q + 2 ] ) && ( s < unambiglength ) ) {
	    ...
	    if ( ( k == bytestart [ q + 2 ] ) && ( choppedid [ s ] != 0 ) ) 
	    goto lab32 ; 
	    {
	      (void) putc('\n',  stdout );
	    ...
	    bytestart [ q + 2 ] - 1 ; if ( k <= for_end) do 
	      (void) putc( xchr [ bytemem [ w ][ k ] ] ,  stdout );
	    while ( k++ < for_end ) ; } 
	    error () ; 
	    q = 0 ; 
...
{/* 2 21 22 */ eightbits curchar  ; 
  ...
  while ( stackptr > 0 ) {
	...
	j = bytestart [ curval ] ; 
	w = curval % 2 ; 
	while ( ( k < maxidlength ) && ( j < bytestart [ curval + 2 ] ) ) {
...
void main_body() {
  ...
  lab9999: if ( stringptr > 128 ) 
  {
    {
      (void) putc('\n',  stdout );
      (void) fprintf( stdout , "%ld%s",  (long)stringptr - 128 ,       " strings written to string pool file." ) ; 
    } 
    (void) putc( '*' ,  pool );
