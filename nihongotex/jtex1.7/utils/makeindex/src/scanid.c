	...
	FIELD_PTR	data;
{
	...
	while (TRUE) {
		...
		if (class == 1)
			...
			class = iskanji(key[n])? 1 : 0;
#endif
		if (key[n] == NULL)
		    	break;
#ifdef KANJI
		if (key[n] == idx_encap && class == 0) {
#else
		if (key[n] == idx_encap) {
#endif
			n++;
#ifdef KANJI
			if (scan_field(&n, &class, data->encap, FALSE, FALSE, FALSE))
#else
...
#ifdef KANJI
		if (key[n] == idx_actual && class == 0) {
#else
		if (key[n] == idx_actual) {
#endif
			n++;
#ifdef KANJI
			if (! scan_field(&n, &class, data->af[i], TRUE, TRUE, FALSE))
#else
			...
			if (i == last) {
#ifdef KANJI
				if (! scan_field(&n, &class, data->sf[i], FALSE, TRUE, TRUE))
#else
...
#ifdef KANJI
				if (! scan_field(&n, &class, data->sf[i], TRUE, TRUE, TRUE))
#else
...
static int
#ifdef KANJI
scan_field(n, class, field, ck_level, ck_encap, ck_actual)
	int		*n;
	int		*class;
	char		field[];
	int		ck_level;
	...
	*class = 0;
	...
	while (TRUE) {
		...
		if (*class == 1)
			...
			*class = iskanji(key[*n])? 1: 0;
		while (key[*n] == idx_escape && *class == 0) {
#else
		...
		if (key[*n] == idx_quote) {
			...
			if (nbsh%2 == 0){
				...
				field[i++] = key[*n];
#ifdef KANJI
		} else if ((ck_level && (key[*n] == idx_level) && *class == 0) ||
			   (ck_encap && (key[*n] == idx_encap) && *class == 0) ||
			   (ck_actual && (key[*n] == idx_actual) && *class == 0) ||
#else
			...
			if ((i > 0) && compress_blanks && (field[i-1] == SPC))
		    	...
		    	field[i++] = key[*n];
#ifdef KANJI
			if ((! ck_level) && (key[*n] == idx_level) && *class == 0) {
#else
			if ((! ck_level) && (key[*n] == idx_level)) {
				...
				return (FALSE);
#ifdef KANJI
			} else if ((! ck_encap) && (key[*n] == idx_encap) && *class == 0) {
#else
				...
				return (FALSE);
#ifdef KANJI
			} else if ((! ck_actual) && (key[*n] == idx_actual) && *class == 0) {
#else
