...
#include <stdio.h>
#include "global.h"
/* #include "config.h"   /* no needing */

/* extern char	*malloc();  /* defined on global.h */
...
#define visible                 /* public attribute */
...
int  width, height;
{
	...
	if ((buf = bufp = (unsigned char *)malloc(size)) == NULL)
	    Fatal("Can't allocate memory in function jxlexp\n");	/* exit */
                       fread(buf, 1, size, fp);
