...
#ifdef NTTJTEX
	struct	pc pd_pc[256];	/* `font directory' */
	int	pd_maxchar;	/* font directory size */
#else /* !NTTJTEX */
	...
	int fd;
{
	...
	if (f->f_flags & FF_RASTERS) {
		...
		(void) lseek(fd, -8L, 2);
		read(fd, &pxldirptr, 4);
		pxldirpos = pxldirptr * 4;
		pxldirsize = ((st.st_size - pxldirpos) / 4 - 5);
		pd->pd_maxchar = pxldirsize / 4;
		i = pxldirpos - 4;
#else /* !NTTJTEX */
...
#ifdef NTTJTEX
	(void) fseek(fp, (long) pxldirptr * 4, 0);
	for (i = pd->pd_maxchar, pc = pd->pd_pc; --i >= 0; pc++) {
#else /* !NTTJTEX */
	...
	f->f_details = (char *)pd;
#ifdef NTTJTEX
	if (FontHasGlyphs(f, 0, pd->pd_maxchar))
#else /* !NTTJTEX */
