...
char **argv;
{
  ...
  for (arg = 1; arg < argc; arg++) {
	...
	if ( nameWithOutPrefix = rindex(name, '/') ) {
	  name = nameWithOutPrefix+1;
	}
	
	if ( Debug ) {
	  fprintf(stderr,"try to open %s with UserMag = %d\n",
		  name, UserMag);
	}
	...
	if (strcmp(name + len - 2,"pk") == 0
	    || strcmp(name + len - 2,"gf") == 0) {
	  ...
	  if ( *p == '.' ) {
	    ...
	    *p = 0;	/* end name*/
	    thisMag = ((value * DviMag * 1000) / DviDpi) / 1000;
	    sDpi = DviDpi * 1000;
	    sDpi /= 1000;
	    
	    if ( Debug || value < (sDpi * 0.25) ||  value > (sDpi * 5) ) {
	      fprintf(stderr, "%s: You have an odd sounding font number\n",
		      ProgName);
	      fprintf(stderr, "%s: I see a %d-dpi from mag %d file magnified %d\n",
		      ProgName, DviDpi, UserMag,thisMag);
	    }
	  }
	}
	else {
	...
	if ( Debug ) {
	  fprintf(stderr,"try to open %s at mag %d\n",
		  name, thisMag);
	}
...
#define MINMAX(x,y) { min_/**/x = MIN(min_/**/x,y); max_/**/x = MAX(max_/**/x,y); }
  ...
  double dsuff = ((double) DviDpi * UserMag * mag)
