...
double shading = -1.0;		/* How should next object be shaded? */
...
static void do_attributes()
{
    ...
    else if (shading >= 0.0 && shading < 0.4) {
	ImWrt1(imP_FILL_PATH);
	ImWrt1(WHITE);
    }
    else if (shading > 0.6) {
...
/*
 * Flush the path that we've built up with im_drawto(), but don't draw it.
 * Called as \special{ip}.
 */
static void im_invispath()
{
    register int i;
...
int dotted;			/* boolean */
{
    ...
    push_location();
    if (dotted) {
	set_pen_size();	/* May want to do something thicker for dotted lines */
 ...
 */
	...
	ImWrt2(0);		/* alphaoff */
	ImWrt2(alpha0);		/* zero start angle */
	ImWrt2(alpha1);		/* two pi end angle */
    }
    if (!invis) {
      ImWrt1(imP_DRAW_PATH);
      ImWrt1(BLACK);
...
char *cp;
{
    ...
    splinelen = 0;
    splinex[splinelen] = xx[1];
    spliney[splinelen++] = yy[1];
    N = pathlen + 1;
    xx[0] = xx[1];
    yy[0] = yy[1];
    xx[N] = xx[N-1];
	...
	for (j = 0; j < steps; j++) {	/* points within */
		...
		if (splinex[splinelen-1] != xp || spliney[splinelen-1] != yp) {
		    splinex[splinelen] = xp;
		    spliney[splinelen++] = yp;
		}
	}
    }
    if ((xx[pathlen] != splinex[splinelen-1] ||
	yy[pathlen] != spliney[splinelen-1]) && splinelen < SPLINEPOINTS) {
	splinex[splinelen] = xx[pathlen];
	spliney[splinelen++] = yy[pathlen];
    }

#define imP_SET_DRAW_PATTERN 237
	ImWrt1(imP_SET_DRAW_PATTERN);
	ImWrt1(2);
	if ( dlen > 0 ) {
		/* do dashed lines */
		ImWrt2(xconv(dlen));
 ...
 *	\special{sh}
 */
static void im_shade(cp)
char *cp;
{
    if (cp && *cp) shading = atof(cp);
    else shading = 0.5;
}
...
int k;
{
    ...
    else if (strcmp(command, "ar") == 0) im_arc(cp, 0);
    else if (strcmp(command, "ia") == 0) im_arc(cp, 1);
    else if (strcmp(command, "sp") == 0) flush_spline(cp);
    else if (strcmp(command, "sh") == 0) im_shade(cp);
    else if (strcmp(command, "wh") == 0) im_shade("0");
    else if (strcmp(command, "bk") == 0) im_shade("1");
    else if (strcmp(command, "tx") == 0) do_texture(cp);
    else if (strcmp(command, "bf") == 0) do_impress(cp);
    else if (strcmp(command, "ip") == 0) im_invispath();
