...
int			inRule;
int			OType = DEFAULT_KANJI_CODE;  
				/* Output Kanji Code type EUC/SJIS/JIS */
...
void dopage()
{
    ...
    while ((opcode = (int) get1()) != EOP) {/* process page until eop reached */
        if (opcode > POST_POST)
            ...
            outchar(opcode);
	else if (171 <= opcode && opcode <= 234)
		SetFntNum(opcode - 171);
        else
            switch (opcode) {
...
void printpage()       /* 'end of page', writes lines of page to output file */
{
    ...
    do {
		...
		if (ch >= 128) {
		      chb = currentline->text[i+1-leftmargin];
		  if (OType==JIS) {
		      if (! inKanji) {
			fprintf(output,KANJI_IN);
			inKanji = TRUE;
		      }
		      fputc(ch-128,output);
		      fputc(chb,output);
		      ...
		      fputc(ch,output);
		      fputc(chb+128,output);
		  } else if (OType==SJIS) {
		    unsigned int jl,jh,sh,sl;
		    jh = ch&0x7f; jl = chb;
		    if (!(jh <= 0x20 || jh >= 0x7f || jl <= 0x20 || jh >= 0x7f)) {
			sh = ((jh - 0x21) >> 1) + 0x81;
			if (sh > 0x9f) {
			   sh += 0x40;
			}
		  ...
		  i++; j++;
		} else if (ch >= SPACE && ch != DEL) {
		  if (inKanji) {
		    fprintf(output,KANJI_OUT);
		    inKanji = FALSE;
		  }
		  putc(ch, output);
		}
...
{
    register int i, j; unsigned int high,low;

    if (abs(v - currentline->vv) > VERTICALEPSILON / 2)
        currentline = findline();
    if (KanjiFont() && ! inRule) { /* NTT TeX case Code Conversion */
       high=ch;
    ...
    if ( (currentline->text[j - leftmargin ]==SPACE ) && (
         ( (ch >= SPACE) && (ch != DEL) ) ||
         (scascii && (ch == 23)) || KanjiFont() || ch > 255)) {
		if (KanjiFont() && ! inRule) {
			if ((j<rightmargin-1) && 
			      (currentline->text[j-leftmargin+1]!=' ')) {
			    currentline->text[j-leftmargin]='*';
			} else {
			    currentline->text[j-leftmargin]=high;
			    currentline->text[j+1-leftmargin]=low;
			  }
		} else if(ch > 255 && ! inRule) {
			if ((j<rightmargin-1) && 
			      (currentline->text[j-leftmargin+1]!=' ')) {
			    currentline->text[j-leftmargin]='*';
			} else {
			    currentline->text[j-leftmargin]=(ch >> 8)|0x80;
			    currentline->text[j+1-leftmargin]=(ch & 0xff);
			  }
		} else {
			if (j < rightmargin)
...
{    /* should print characters with character code>127 from current font */
    ...
    /*  But this is no true for Kanji Character.                          */
    outchar(charnr);
} /* setchar */
